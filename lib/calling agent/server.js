...
const twilioNumber = process.env.TWILIO_PHONE_NUMBER || "(API)";
const defaultCallNumber = process.env.DEFAULT_CALL_NUMBER || "(API)";
...
app.post('/call-user', async (req, res) => {
  try {
    ...
    // Use specific ngrok webhook URL for our server
    const callWebhookUrl = `https://7d31-2409-40c0-100d-2ac8-e1eb-2bb6-80e9-5a11.ngrok-free.app/voice`;
    if (ngrokUrl.includes('demo.twilio.com')) {
      console.log("📞 Using custom webhook for conversation flow");
    }
    
    console.log(`📞 Initiating real Twilio call to: ${phoneNumber}`);
    
    // Create call with embedded TwiML (no webhook needed)
    const call = await twilioClient.calls.create({
      ...
      twiml: `<Response>
        <Say voice="alice" rate="slow" language="en-US">
          ...
          I will help you file a complaint just like our web chatbot.
          
          To file a complaint online, please visit our website.
          For phone support, please call during business hours.
          
          Thank you for using our Grievance Portal.
          Your call is important to us.
          
          Press any key to repeat this message, or hang up to end the call.
        </Say>
        
        <Gather input="dtmf" timeout="5" numDigits="1">
          <Say voice="alice" rate="slow">Press any key to repeat, or hang up to end.</Say>
        </Gather>
        
        <Say voice="alice" rate="slow">Thank you for calling. Goodbye!</Say>
        <Hangup/>
      </Response>`,
      timeout: 30
    });
